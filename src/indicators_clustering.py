...
def define_defaults():
    ...
    energyPenalty = -0.5 #-0.5 is equivalent having 1/v as an indicator
    dpi = 100
...
def read_energy_and_indicators(dataDict):
    ...
    elif materialOpt == 'HEU_MET_FAST_085_3':
        importanceDict = {'HEU85': 10, 'SteelyKnife': 1} 
    elif materialOpt == 'HEU_MET_FAST_019':
        importanceDict = {'HEU19': 10, 'Carbon12': 1} 
    elif materialOpt == 'HMF019':
        importanceDict = {'HEU019': 1} 

    elif materialOpt == 'UAgAu':
        importanceDict = {'Natural_Ag': 1.0, 'Pure_U235': 1.0, 'Natural_Au': 1.0} 
    elif materialOpt == 'heu_water':
        importanceDict = {'HMF001': 1} 
    elif materialOpt == 'HMF001':
        importanceDict = {'HMF001': 1} 
    elif materialOpt == 'AlAgAu':
        importanceDict = {'Natural_Ag': 1.0, 'Natural_Au': 1.0, 'Natural_Al': 1.0} 
    elif materialOpt == 'Natural_Iron':
        importanceDict = {'Natural_Iron': 1} 
    elif materialOpt == 'HEU20':
        importanceDict = {'HEU20': 1} 
...
def compute_observations(dataDict, indicators):
    ...
    if energyPenalty < 0:
        numMaterials = np.float(numIndicators-1)/2. # divided by 2 because each material has 2 indicators

        # energyPenalty = -0.5 can be derived by solving the transport equation for a time step
        # energyPenalty = -0.5 is equivalent having 1/v as an indicator
        observations[-1,:] *= np.sqrt(numMaterials) * energyPenalty
        # (obsRange / energyRange) is not derived from the transport eq., so I didn't use it
        ...
        # Old energy penalty multiplication factor
...
def auto_apportion(observations, numElementsRRR, groupBdrs, coarseBdrs, apportionOpt, verbosity):
    ...
    numCoarseGroups = len(coarseBdrs) - 1
    cg_dLogE = np.zeros(numCoarseGroups)
...
def write_mesh(dataDict, globalLabels):
    ...
    # XML Output
    clust = np.loadtxt(filePath, skiprows=2, usecols=[0,1])
    mapping_sub_to_elem = get_mapping_sub_to_elem(clust)
    mapping_elem_to_subs = get_reverse_mapping_for_array(mapping_sub_to_elem)

    with open(filePath[:-4]+'.xml', 'w') as f:
        f.write('<clust>\n')
        f.write('  <map>\n')
        for e in range(len(mapping_elem_to_subs)):
            f.write('    <element_%i>\n      ' %e)
            for s_in_e in range(len(mapping_elem_to_subs[e])):
                f.write('%i' %mapping_elem_to_subs[e][s_in_e] )
                if s_in_e != (len(mapping_elem_to_subs[e])-1):
                    f.write(', ') 
            f.write('\n    </element_%i>\n' %e)
        f.write('  </map>\n')
        f.write('  <eBounds >\n    ')            
        for s in range(len(clust)):
            f.write('%1.14e' %(1e-6*clust[s,1]) )
            if s != (len(clust)-1):
                f.write(', ')  
        f.write('\n  </eBounds >\n')  
        f.write('</clust>\n')
...
def define_input_parser():
    ...
    parser.add_argument('-m', '--materialopt', help="Unless 'manual' is used, specifies a set of materials to use. If 'manual' is used, give a space-separated list of material names in 'listmaterials'.", choices=['HEU_MET_FAST_001','HEU20','Natural_Iron','AlAgAu','HMF001','heu_water','UAgAu','HMF019','HEU_MET_FAST_019','HEU_MET_FAST_085_3','4','5','c5g7', 'graphite', 'iron', 'kpin', 'kenrichedpin', 'kcladpin', 'kpin2d', 'kenrichedpin2d', 'kmoxpin2d', 'kmoxenrichedpin2d', 'CASL', 'manual'], default=defaults['materialopt'])
