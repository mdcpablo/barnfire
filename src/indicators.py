...
def do_all(inputDict):
    ...
    min_rel_diff = inputDict['min_rel_diff']
    ...
    if materialOpt == '3':
        ...
        materials.append(mat.get_HEU85_material())
        materials.append(mat.get_SteelyKnife_material())
        if workOpt == 'wgt':
            materials.append(mat.get_UnpolutedAir_material())
    elif materialOpt == 'HEU_MET_FAST_019':
        materials.append(mat.get_HEU19_material())
        materials.append(mat.get_Carbon12_material())
        if workOpt == 'wgt':
            materials.append(mat.get_UnpolutedAir_material())
    elif materialOpt == 'HMF019':
        materials.append(mat.get_HEU019_material())
        if workOpt == 'wgt':
            materials.append(mat.get_Graphite019_material())
    elif materialOpt == 'UAgAu':
        materials.append(mat.get_Pure_U235_material())
        materials.append(mat.get_Natural_Ag_material())
        materials.append(mat.get_Natural_Au_material())
    elif materialOpt == 'heu_water':
        materials.append(mat.get_HMF001_material())
        if workOpt == 'wgt':
            materials.append(mat.get_H2O_material())
    elif materialOpt == 'HMF001':
        materials.append(mat.get_HMF001_material())
    elif materialOpt == 'AlAgAu':
        materials.append(mat.get_Natural_Al_material())
        materials.append(mat.get_Natural_Ag_material())
        materials.append(mat.get_Natural_Au_material())
        if workOpt == 'wgt':
            materials.append(mat.get_Pure_U235_material())
    elif materialOpt == 'HEU20':
        materials.append(mat.get_HEU20_material())
    elif materialOpt == 'HEU_MET_FAST_001':
        materials.append(mat.get_Godiva_material())
...
def set_minimum_difference(xj, yij, min_abs_diff=1e-8, min_rel_diff=1e-8):
    '''
    This function removes points that are within 'min_abs_diff' or 'min_rel_diff' of each other.
    This function was added because NJOY sometimes errors out because group bin sizes
    are too small, and NJOY then thinks that the group bins are out of order.
    (This function is called by the function "thicken_grid")   
    '''
    xj_str = []   
    yij_str = []
    ...
    for j in range(len(xj)):
        if (xj[j] - prev_xj)/prev_xj > min_rel_diff and (xj[j] - prev_xj) > min_abs_diff: 
            xj_str.append( xj[j] )
            for i in range(len(yij)):
                yij_str[i].append( yij[i,j] )
            prev_xj = xj[j]    
        else:  
            num_points_removed += 1   
    print '** removed', num_points_removed, 'points because they were too close to each other'
    
    xj = np.array(xj_str)
    yij = np.array(yij_str) 
    return xj, yij
...
def thicken_grid(xj, yij, maxRelJump, opt='x', verbosity=False):
    ...
    while pointsAdded:
        iteration += 1
        if iteration % 500 == 0: 
            print '***** thicken_grid, while pointsAdded, iteration #', iteration 
            print '      numToAdd =', numToAdd 
        if iteration == 1000:
            print 'IF THERE ARE TOO MANY ITERATIONS:'
            print '  1) Check version of NJOY being used'
            print '  2) Try regenerating PENDF and GENDF files from scratch'
        if opt == 'x':
    ...
    if verbosity:
        print 'Enlarged grid from {0} to {1} points in {2} iterations'.format(
            originalNumPoints, numPoints, iteration)
    xj[0] = initialX
    (xj, yij) = set_minimum_difference(xj, yij, min_rel_diff=inputDict['min_rel_diff'])
    return xj, yij
...
def save_material_fluxes(materials, energyGrid, fluxMat, groupBdrs, pwResFactor, workOpt, plotOutput=False):
    ...
    with open(os.path.join(datDirr,'flux.xml'), 'w') as f:
        f.write('<flux_v_energy>\n')
        f.write('  <eBounds>\n    ')            
        for s in range(len(mgEnergyGrid)):
            f.write('%1.14e' %(1e-6*mgEnergyGrid[s]) )
            if s != (len(mgEnergyGrid)-1):
                f.write(', ')  
        f.write('\n  </eBounds>\n')  
        f.write('  <fluxes>\n')    
        for iMat, material in enumerate(materials):
            materialName = material.shortName       
            f.write('    <%s>\n      ' %materialName)
            for s in range(len(mgFluxMat[iMat])):
                f.write('%1.14e' %mgFluxMat[iMat,s] )
                if s != (len(mgFluxMat[iMat])-1):
                    f.write(', ')   
            f.write('\n    </%s>\n' %materialName)
        f.write('  </fluxes>\n')  
        f.write('</flux_v_energy>\n')
...
def define_input_parser():
    ...
    parser.add_argument('-s', '--min_rel_diff', help='The minimum energy bin width. This is a relative difference. For example, "-s 0.01" would mean that if the lower bound for an energy group is 1.00 eV then the upper bound must be at least 1.01 eV... or if the lower bound for an energy group is 5.00 keV then the upper bound must be at least 5.05 keV.', type=float, default=defaults['min_rel_diff'])
