...
BE CAREFUL, "driver.py" may delete or add lines from the following files in the Barnfire source code:
1) indicators.py
...
1) add functions to "driver.py"
1) modify the bash scripts (generated by "driver.py") before running them
...
def create_PDT_xs(prob, geom):
    ...
    if prob.element_type in ['mg','MG']:
        ...
        if prob.create_MCNP_cross_sections == True or prob.only_create_MCNP_cross_sections == True:
            f3.write("# Step 2b: Run NJOY to generate the ACE files (can be done after step 2)\n")
            f3.write("cd $SCRATCH_BARN/xs\n")
            f3.write("./RunAce.sh\n")
            f3.write("# Step 2c: Copy ACE files and create xsdir\n")
            f3.write("cd ace/xdata\n")
            f3.write("./copyAce.sh\n")
            f3.write("cd $scriptdir\n")
            f3.write("# The result should be ACE files and xsdir (tells MCNP where the ACE files are) in $SCRATCH_BARN/xs/ace/xdata\n")
            f3.write("cp $SCRATCH_BARN/xs/ace/xdata/*.90c $scriptdir/%s/.\n" %prob.dir )
            f3.write("cp xsdir $scriptdir/%s/xsdir_%s\n\n" %(prob.dir, prob.dir) )

        if prob.only_create_MCNP_cross_sections == False:
