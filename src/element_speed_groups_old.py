...
def get_mapping_elem_to_sub(mapping_sub_to_elem):
    mapping_elem_to_sub = []
    for i in range(int(max(mapping_sub_to_elem))+1):
       mapping_elem_to_sub.append([])
    for i in range(len(mapping_sub_to_elem)):
        mapping_elem_to_sub[mapping_sub_to_elem[i]].append(i)
    return mapping_elem_to_sub

def get_elem_sg(coarseBdrs, elem, sub_wgt, elem_wgt, n):
    '''Inserts integrated-flux-spaced group boundaries within each coarse group'''  
    sub_over_elem_wgt = np.zeros(max(elem)+1)
    local_num_of_elem_sg = np.zeros(max(elem)+1)
    local_num_of_elem_sg[0] += 1
    elem_sg = np.zeros(len(elem), dtype=int)
    prev_elem = [0] 
    #print elem[0], elem_sg[0], sub_over_elem_wgt[elem[0]]
    for i in range(1,len(elem)):
        sub_over_elem_wgt[elem[i]] += sub_wgt[i]/elem_wgt[i]
        if elem[i] not in prev_elem:
            elem_sg[i] = max(elem_sg) + 1
            local_num_of_elem_sg[elem[i]] += 1
            prev_elem.append(elem[i])
        else:
            if sub_over_elem_wgt[elem[i]] < (1-1e-8):
                for m in range(1,n):
                    if sub_over_elem_wgt[elem[i]] > np.float(m)/np.float(n):
                        if local_num_of_elem_sg[elem[i]] < n:
                            #print local_num_of_elem_sg[elem[i]] , n
                            elem_sg[i] = max(elem_sg) + 1
                            local_num_of_elem_sg[elem[i]] += 1
                            prev_elem.append(elem[i])
                            break
                ...
                elem_sg[i] = max(elem_sg) - (max(prev_elem) - elem[i])
        #print elem[i], elem_sg[i], sub_over_elem_wgt[elem[i]]
    print "# elements =", max(elem)+1, "\n# speed groups =", int(sum(local_num_of_elem_sg)), "\nlocal_num_of_elem_sg =",  local_num_of_elem_sg
    return elem_sg

def get_elem_sg_1(coarseBdrs, sub_ehigh, elem, sub_wgt, elem_wgt, n):
    '''Inserts integrated-flux-spaced group boundaries within each coarse group'''  
    sub_over_elem_wgt = np.zeros(max(elem)+1)
    local_num_of_elem_sg = np.zeros(max(elem)+1)
    local_num_of_elem_sg[0] += 1
    ...
    elem_sg_high_ehigh = np.zeros(int(max(elem_sg))+1)
    elem_sg_dLogE = np.zeros(int(max(elem_sg))+1)

    prev_elem = []
    prev_elem_sg = []

    cg_de = np.zeros(int(max(elem_sg))+1)
    cg_dLogE = np.zeros(int(max(elem_sg))+1)
    ...
    for i in range(len(elem_sg)):
        ...
        if elem_sg[i] not in prev_elem_sg:
            elem_sg_high_ehigh[elem_sg[i]] = np.copy(sub_ehigh[i])
            prev_elem_sg.append(elem_sg[i])
            if coarseBdrs[-1] < sub_ehigh[i]-1e-14 < coarseBdrs[0]:
                for j in range(len(coarseBdrs)-1):
                    if coarseBdrs[j+1] < sub_ehigh[i]-1e-14 < coarseBdrs[j]:
                        cg_de[elem_sg[i]] = coarseBdrs[j] - coarseBdrs[j+1]
                        cg_dLogE[elem_sg[i]] = np.log10(coarseBdrs[j]) - np.log10(coarseBdrs[j+1])
         
        else:
            if i != len(elem_sg):
                elem_sg_dLogE[elem_sg[i]] = np.log10(elem_sg_high_ehigh[elem_sg[i]]) - np.log10(sub_ehigh[i+1])

    for i in range(len(mapping_sg_to_elem)):
        print i, mapping_sg_to_elem[i], elem_sg_dLogE[i]/cg_dLogE[i]

    mapping_sg_to_newsg = np.zeros(int(max(elem_sg))+1, dtype=int)
    j = -1
    for i in range(len(mapping_sg_to_newsg)):
        if 1e-15 < elem_sg_dLogE[i]/cg_dLogE[i] < 1./np.float(n):
            a = 1
            #print elem_sg_dLogE[i]/cg_dLogE[i], " < ",  1./np.float(n), "... removing speed group", i
        else:
            j += 1
        print i, j, elem_sg_dLogE[i]/cg_dLogE[i]
        #mapping_sg_to_newsg[i] = np.copy(j)
        #print mapping_sg_to_elem[i], mapping_sg_to_newsg[i]
...
def get_elem_sg_3(coarseBdrs, sub_ehigh, elem, sub_wgt, elem_wgt, n):
    '''Inserts integrated-flux-spaced group boundaries within each coarse group'''  
    sub_over_elem_wgt = np.zeros(max(elem)+1)
    local_num_of_elem_sg = np.zeros(max(elem)+1)
    local_num_of_elem_sg[0] += 1
        ...
        if len(mapping_elem_to_sub[e]) > 1:
            ...
            for i in range(len(elem_split_locations)):
                if i == 0:
                    ...
                    LogSpeedSpacing_OnLeft = (np.log(ehigh[elem_split_locations[i-1]]) - np.log(elow[elem_split_locations[i]]))/cg_dLogE[e]
                ...
                '''if LogSpeedSpacing_OnLeft + LogSpeedSpacing_OnRight < 1./np.float(n):
                    #print "rejecting split location", elem_split_locations[i]
                    improved_elem_split_locations.remove(elem_split_locations[i])
                '''

                if LogSpeedSpacing_OnLeft < 0.5/np.float(n) or LogSpeedSpacing_OnRight < 0.5/np.float(n):
                    #print "rejecting split location", elem_split_locations[i]
                    improved_elem_split_locations.remove(elem_split_locations[i])

            elem_split_locations = copy.copy(improved_elem_split_locations)
            print elem_split_locations
            ...
            for i in range(len(elem_split_locations)):
                ...
                print LogSpeedSpacing_OnLeft, elem_split_locations[i], LogSpeedSpacing_OnRight
            print 
    ...
    for e in range(len(mapping_elem_to_sg)):      
        ...
        for sg in mapping_elem_to_sg[e]:
            print sg, mapping_sg_to_sub[sg]
        print mapping_elem_to_sub[e]

    for sg in range(len(mapping_sg_to_sub)):
        for sub in range(len(mapping_sg_to_sub[sg])):
            elem_sg[mapping_sg_to_sub[sg][sub]] = np.copy(sg)
...
def get_elem_sg_4(coarseBdrs, sub_ehigh, elem, sub_wgt, elem_wgt, n):
    '''Inserts integrated-flux-spaced group boundaries within each coarse group'''  
    sub_over_elem_wgt = np.zeros(max(elem)+1)
    local_num_of_elem_sg = np.zeros(max(elem)+1)
    local_num_of_elem_sg[0] += 1
    ...
    for e in range(len(mapping_elem_to_sub)): 
        ...
        if len(mapping_elem_to_sub[e]) > 1:
            exit = False
            print "----------------------------------------"
            for max_num_sg in range(n, 0, -1):
                if exit == True:
                    break
                elem_split_locations = []
                for m in range(1,max_num_sg):
                    #print elem_split_locations
                    print elem_sum_wgts[e]
                    print np.float(m)/np.float(max_num_sg)
                    distances = [np.abs(elem_sum_wgts[e][s] - np.float(m)/np.float(max_num_sg)) for s in range(len(elem_sum_wgts[e]))]
                    #print distances
                    for s in range(len(elem_sum_wgts[e])):
                        if np.abs(distances[s] - min(distances)) < 1e-14:
                            if s not in elem_split_locations and s != 0:
                                elem_split_locations.append( s )
                                break
                print max_num_sg, elem_split_locations                         
                ehigh = np.zeros(len(elem_sum_wgts[e]))
                elow = np.zeros(len(elem_sum_wgts[e]))
                for s in range(len(elem_sum_wgts[e])):
                    ehigh[s] = sub_ehigh[mapping_elem_to_sub[e][s]]
                ...
                for i in range(len(elem_split_locations)):
                    ...
                    if LogSpeedSpacing_OnLeft > 0.5/np.float(n) and LogSpeedSpacing_OnRight > 0.5/np.float(n):
                        exit = True
...
def calc_group_total_values(sub_vals, group_number):
    #print sub_vals, group_number
    #print len(sub_vals), len(group_number), max(group_number)
    group_vals_small_array = np.zeros(max(group_number)+1)
    for i in range(len(sub_vals)):
        group_vals_small_array[group_number[i]] += sub_vals[i]
    group_vals = np.zeros(len(sub_vals))
    for i in range(len(sub_vals)):
        group_vals[i] += group_vals_small_array[group_number[i]]
    return group_vals

def calc_group_averaged_values(sub_vals, group_number, sub_wgt, group_wgt):
    group_vals_small_array = np.zeros(max(group_number)+1)
    for i in range(len(sub_vals)):
        group_vals_small_array[group_number[i]] += sub_vals[i] * (sub_wgt[i] / group_wgt[i])
    group_vals = np.zeros(len(sub_vals))
    for i in range(len(sub_vals)):
        group_vals[i] += group_vals_small_array[group_number[i]]
        #print group_number[i], sub_vals[i], group_vals[i]
    return group_vals
###############################################################################
def save_material_speeds(materials, meshName, pwResFactor, coarseBdrs, apportionOpt, numElements, numSpeedGroups):
    ...
    mapping_elem_to_sub          = get_mapping_elem_to_sub(mapping_sub_to_elem)
    ...
    for max_num_speed_groups in numSpeedGroups:
        ...
        get_elem_sg_4(coarseBdrs, sub_ehigh, elem, sub_wgt, elem_wgt, max_num_speed_groups)
        elem_sg_de    = calc_group_total_values(sub_de, elem_sg)
        elem_sg_wgt   = calc_group_total_values(sub_wgt, elem_sg)
        elem_sg_v     = calc_group_averaged_values(sub_v, elem_sg, sub_wgt, elem_sg_wgt)     
        ...
        matrix_to_save = [elem, sub_de, elem_sg, elem_sg_de, elem_sg_v]
        header_to_print = 'element_number,       subelement_de,         element_speed_group_number,  element_speed_group_de,   element_speed_group_speed, '
    ...
    plt.scatter(elem_sg_v,2.*np.ones(len(elem_v)),color='r',marker='|')
    plt.xscale('log')
    plt.xlim([1e8,1e10])
    plt.ylim([0,10])
    plt.show()
...
def define_input_parser():
    ...
    parser.add_argument('-m', '--materialopt', help=" Unless 'manual' is used, specifies a set of materials to use. If 'manual' is used, give a space-separated list of material names in 'listmaterials'.", choices=['HEU','Pure_U235','Iron_and_U235','Natural_Iron','Pu239_and_Air','3','4','5','c5g7', 'graphite', 'iron', 'kpin', 'kenrichedpin', 'kcladpin', 'kpin2d', 'kenrichedpin2d', 'kmoxpin2d', 'kmoxenrichedpin2d', 'CASL', 'deb', 'manual'], default=defaults['materialopt'])
