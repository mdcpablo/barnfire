import sys
#sys.path.append('/home/pablo/barnfire/examples')
...
geom.Name = 'UAgAu' # CHANGE THE MATERIAL NAME HERE if you decide to make changes to a material's definition or specifications
...
geom.importantMats = [Pure_U235, Natural_Ag, Natural_Au] # list of materials that affect your weighting spectrum in important regions of the geometry
geom.importanceDict = {'Pure_U235': 1., 'Natural_Ag':1., 'Natural_Au':1.} # relative importance of each material for your clustering algorithm
...
prob_1.name = 'UAgAu_equal_weighting'
###############################################################################
...
for mg in [100]:
    ...
    prob_1.dir = (prob_1.name+'_%img' %mg) # directory name that will contain Barnfire output and cross sections files
...
for feds in [100]:
    ...
    prob_1.dir = (prob_1.name+'_%ifeds' %feds) # directory name that will contain Barnfire output and cross sections files
    run_barnfire_driver(prob_1, geom)
...
for feds in [100]:
    ...
    prob_1.num_elements_RRR = feds-2
    prob_1.dir = (prob_1.name+'_%ifeds_new' %feds) # directory name that will contain Barnfire output and cross sections files
    run_barnfire_driver(prob_1, geom)
################################################################################
prob_1.element_type = 'FEDS'
prob_1.time_dependent = False
