...
H.name = 'h' # lower case 
H.Name = 'H' # upper case
H.AtomFractionsDict = {1: 1} # atom fractions (or number densities) of each isotope (e.g. {1: 0.5, 2: 0.5} means equal number of H-1 and H-2 atoms)
 # atom fractions (or number densities) of each isotope (e.g. {1: 0.5, 2: 0.5} means equal number of H-1 and H-2 atoms)
...
H2O = Material() 
H2O.Name = 'H2O' # CHANGE THE MATERIAL NAME HERE if you decide to make changes to a material's definition or specifications
H2O.temperature = 293.6 # Kelvin
H2O.elements = [H, O] # list of elements that your material is composed of
H2O.elemAtomFracDict = {'H': 2., 'O': 1.} # atom fractions (or number) of each isotope (e.g. {'H': 2, 'O': 1} is water)
H2O.massDensity = 1. # grams/cm^3
H2O.fuelRadius = 0. # if mat.fuelRadius is set to 0., Barnfire will generate infinite medium xs
H2O.shape = 'slab' # if mat.shape is not 'sphere', 'cylinder', or 'cube', Barnfire will generate infinite medium xs

# add comment about why uranium isn't important
geom = Material_Geometry()
geom.Name = 'heu_water' # CHANGE THE MATERIAL NAME HERE if you decide to make changes to a material's definition or specifications
geom.unimportantMats = [H2O] # list of materials that have a negligible effect on your weighting spectrum in important regions of the geometry
...
prob_1.plot_fluxes = False
for mg in [5, 10, 25, 50, 100, 250, 500, 1000]:
    prob_1.num_mid_E_bins = mg-2
    prob_1.num_elements_RRR = mg-2
    prob_1.dir = ('heu_water_%img' %mg) # directory name that will contain Barnfire output and cross sections files
