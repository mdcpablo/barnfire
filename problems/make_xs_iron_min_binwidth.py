...
def create_PDT_xs(prob, geom):
    ...
    f3.write(" \n")
    for mod in geom.ModList:
        f3.write("cp $SCRATCH_BARN/xs/pdtxs/xs_%s_%i.data $scriptdir/%s_%it_%ir_%ib_%iu.cx \n" %(mod.Name, num_FEDS_groups, mod.Name, prob.num_low_E_bins, prob.num_mid_E_bins, prob.num_bands_RRR, prob.num_high_E_bins) )
    for fuel in geom.FuelList:
        f3.write("cp $SCRATCH_BARN/xs/pdtxs/xs_%s_%i.data $scriptdir/%s_%it_%ir_%ib_%iu.cx \n" %(fuel.Name, num_FEDS_groups, fuel.Name, prob.num_low_E_bins, prob.num_mid_E_bins, prob.num_bands_RRR, prob.num_high_E_bins) )
    for mod in geom.ModList:
        f3.write("cp $SCRATCH_BARN/dat/indicators/wgt_%s_$res.txt $scriptdir/%s_%it_%ir_%ib_%iu.wgt \n" %(mod.Name, mod.Name, prob.num_low_E_bins, prob.num_mid_E_bins, prob.num_bands_RRR, prob.num_high_E_bins) )
    for fuel in geom.FuelList:
        f3.write("cp $SCRATCH_BARN/dat/indicators/wgt_%s_$res.txt $scriptdir/%s_%it_%ir_%ib_%iu.wgt \n" %(fuel.Name, fuel.Name, prob.num_low_E_bins, prob.num_mid_E_bins, prob.num_bands_RRR, prob.num_high_E_bins) )
    f3.write("#######################\n")
...
Fe.name = 'fe'
Fe.Name = 'Fe'
Fe.AtomFractionsDict = {54:8.68307E-06, 56:1.36306E-04, 57:3.14789E-06, 58:4.18926E-07}

Natural_Iron = Material()
Natural_Iron.Name = 'Natural_Iron'
Natural_Iron.temperature = 293.6
Natural_Iron.elements = [Fe]
Natural_Iron.elemAtomFracDict = {'Fe': 1.}
Natural_Iron.massDensity = 7.874

U235 = Element()
U235.name = 'u'
U235.Name = 'U'
U235.AtomFractionsDict = {235: 1.}

Pure_U235 = Material()
Pure_U235.Name = 'Pure_U235'
Pure_U235.temperature = 293.6
Pure_U235.elements = [U235]
Pure_U235.elemAtomFracDict = {'U': 1.}
Pure_U235.massDensity = 18.39374

geom = Material_Geometry()
geom.Name = 'Iron_and_U235'
geom.ModList = [Natural_Iron]
geom.FuelList = [Pure_U235]
geom.importanceDict = {Pure_U235.Name: 1}
...
#prob_1.range_bounds = [1e-5, 1.77828, 31622.8, 1.42e7]
prob_1.range_bounds = [1e-5, 1e4, 5e5, 1.42e7]
prob_1.log_spacing = True
prob_1.num_low_E_bins = 1
prob_1.num_mid_E_bins = 100
prob_1.num_high_E_bins = 1400

prob_1.element_type = 'FEDS'
prob_1.num_bands_RRR = 200
prob_1.min_binwidth = 1e-16

prob_1.create_multigroup_structure = True
prob_1.create_PDT_cross_section = True
prob_1.new_PENDF = False
prob_1.new_GENDF = True
prob_1.start_at_step = 9
prob_1.run_PDT_cross_section = True
prob_1.scat_leg_order = 7
prob_1.apportion_algorithm = 'var'
prob_1.rxnOpt = 'invel' #'total'
prob_1.plot_fluxes = False
prob_1.create_MCNP_cross_sections = False
...
#prob_1.range_bounds = [1e-5, 1.77828, 31622.8, 1.42e7]
prob_1.range_bounds = [1e-5, 1e4, 1e6, 1.42e7]
prob_1.log_spacing = True
prob_1.num_low_E_bins = 11
prob_1.num_mid_E_bins = 200
prob_1.num_high_E_bins = 1000

prob_1.element_type = 'FEDS'
prob_1.num_bands_RRR = 400
prob_1.min_binwidth = 1e-16
